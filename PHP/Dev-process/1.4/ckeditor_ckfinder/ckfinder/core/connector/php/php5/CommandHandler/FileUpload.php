 ...
 */
if (!defined('IN_CKFINDER')) exit('Access Denied!');
...
class CKFinder_Connector_CommandHandler_FileUpload extends CKFinder_Connector_CommandHandler_CommandHandlerBase
{
    ...
    public function sendResponse()
    {
        ...
        $sFileName=date(Ymd)."_".date(His).rand(0,10000).".".$sExtension;//对上传的文件进行重命名
        ...
        while (true)
        {
            ...
            if (file_exists($sFilePath)) {
                ...
                if (false === move_uploaded_file($uploadedFile['tmp_name'], $sFilePath)) {
					...
					$query="insert into `news2` (title,content) values('$sFileName','空')";
					mysql_query($query);
					//insert mysql
                }
