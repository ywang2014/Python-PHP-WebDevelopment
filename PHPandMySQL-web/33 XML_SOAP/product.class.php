...
class Product
{
	...
	function __construct($xml)
	{
		if (METHOD == 'SOAP')
		{
			...
			if (is_array($xml['SimilarProducts']['SimilarProduct']))
			{
				foreach ($xml['SimilarProducts']['SimilarProduct'] as $similar)
				{
					...
					$this->similarProducts[$similarProductCount]['ASIN'] = $review['ASIN'];	// $similar['ASIN'] ?
					$similarProductCount++;
				}
			}
		}
		else
		{
			...
			$this->outPrice = (string)$xml->OfferSummary->LowestNewPrice->FormattedPrice;
			$this->outPrice = str_replace('$', '', $this->outPrice);
			$this->outPrice = str_replace(',', '', $this->outPrice);
			$this->outPrice = floatval($this->outPrice);
			...
			if ($xml->CustomerReviews->Review)
			{
				foreach ($xml->CustomerReviews->Review as $review)
				{
					$this->reviews[$reviewCount]['Rating'] = (float)$reviews->Rating;
